...
describe('Access user file list', async function() {
    ...
    fileList = []
    for (let i = 0; i < 3; i++) {
        await fs.copyFile(`${__dirname}/test_files/input_file.krn`,"../uploads/123/input_file"+i+".krn", (err) =>{
            if(err) throw err;
        });
        fileList.push("input_file"+i+".krn")
    }
    ...
    it('should return the file list', async function() {
        ...
        for (let i = 0; i < 3; i++) {
            fs.unlinkSync(userPath+"input_file"+i+".krn");
        }
    });
